...
async def handle_client_command(msg: dict, ws: WebSocket):
    ...
    if cmd == "buy":
        ...
        await ws.send_text(json.dumps({"type": "order_result", "cmd": "buy", "result": result, "ts": datetime.now().isoformat()}, ensure_ascii=False, default=str))
    elif cmd == "sell":
        result = engine_ref.api.send_order(code=msg["code"], order_type=2, quantity=msg["qty"], price=msg["price"], quote_type="00")
        await ws.send_text(json.dumps({"type": "order_result", "cmd": "sell", "result": result, "ts": datetime.now().isoformat()}, ensure_ascii=False, default=str))
    elif cmd == "market_buy":
        result = engine_ref.api.send_order(code=msg["code"], order_type=1, quantity=msg["qty"], price=0, quote_type="03")
        await ws.send_text(json.dumps({"type": "order_result", "cmd": "market_buy", "result": result}, ensure_ascii=False, default=str))
        ...
        if holding:
            ...
            await ws.send_text(json.dumps({"type": "order_result", "cmd": "emergency_sell", "result": result}, ensure_ascii=False, default=str))
    elif cmd == "cancel_all":
        ...
        await ws.send_text(json.dumps({"type": "config_changed", "auto_trading": engine_ref.auto_trading_enabled}, ensure_ascii=False, default=str))
    elif cmd == "set_regime":
        engine_ref.chart1.regime = msg["regime"]
        await ws.send_text(json.dumps({"type": "config_changed", "regime_override": msg["regime"]}, ensure_ascii=False, default=str))
